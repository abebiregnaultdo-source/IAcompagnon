...
export default function CreationCard({ creation }) {
  const getTypeIcon = (type) => {
    const icons = { text: "📝", audio: "🎙️", drawing: "🎨", photo: "📷", voice: "🎙️", image: "📷" };
    return icons[type?.toLowerCase()] || "✨";
  };
  const getTypeLabel = (type) => {
    const labels = { text: "Texte", audio: "Audio", voice: "Audio", drawing: "Dessin", photo: "Photo", image: "Photo" };
  ...
  const getRelativeTime = (date) => {
    ...
    return createdDate.toLocaleDateString("fr-FR", { day: "numeric", month: "long" });
  };
  ...
  const renderContent = () => {
    ...
    switch (type) {
      case "text":
        return (
          <div style={{ maxHeight: 80, overflow: "hidden", lineHeight: 1.6 }}>
            {(creation.content || creation.excerpt || creation.text || "").substring(0, 150)}
            {(creation.content || creation.excerpt || creation.text || "").length > 150 ? "..." : ""}
          </div>
        );
      case "audio":
      case "voice":
        return (
          <>
            <div className="helo-audio-bar">
              <div className="helo-audio-fill" style={{ width: `${creation.progress || 0}%` }} />
            </div>
      ...
      case "image":
        if (creation.imageUrl || creation.image_url || creation.url) {
          return (
            <img
              src={creation.imageUrl || creation.image_url || creation.url}
  ...
  const getActionLabel = () => {
    const type = creation.type?.toLowerCase();
    const labels = { text: "Lire", audio: "Écouter", voice: "Écouter", drawing: "Voir", photo: "Voir", image: "Voir" };
