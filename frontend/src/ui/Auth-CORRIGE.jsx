...
export default function Auth({ onAuthenticated }) {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      // ✅ CORRECTION : Créer utilisateur SANS first_name, tone, rhythm
      // Pour forcer l'affichage de l'onboarding
      const newUser = {
        id: Math.random().toString(36).slice(2) + Date.now().toString(36),
        email,
        password, // ⚠️ En production : hasher le mot de passe!
        created_at: new Date().toISOString(),
        // PAS DE first_name, tone, rhythm, active_module
        // → L'onboarding les demandera
      };
